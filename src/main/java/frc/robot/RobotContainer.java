...
import frc.robot.commands.auto_routines.FetchBunnyAndShoot;
...
public class RobotContainer {
    ...
    // private final SendableChooser<Supplier<Command>> autoChooserShooter = new
    // SendableChooser<Supplier<Command>>();
    ...
    private void configureAutoChoosers() {
        autoChooser.addOption("Fetch bunny",
                new FetchBunny(drivetrain, grabber).withName("Fetch Bunny"));

        autoChooser.addOption("Fetch bunny and shoot",
                new FetchBunnyAndShoot(drivetrain, grabber, turret, indexer, shooter)
                        .withName("Fetch Bunny and shoot"));
    ...
    private static boolean autoHasRun = false;
