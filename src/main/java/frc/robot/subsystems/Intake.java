...
public class Intake extends SubsystemBase {
    ...
    public Intake() {
        intakeMotor = new CANSparkMax(CANConfig.INTAKE_WHEELS,
                MotorType.kBrushless);

        intakeMotor.restoreFactoryDefaults();
        intakeMotor.setInverted(false);

        ShuffleboardHelper.addOutput("Top Output", Constants.INTAKE_TAB,
                () -> intakeMotor.get());
    ...
    /** Spins intake wheels and belt to intake balls. */
    public Command spinForward() {
        return Commands.runOnce(() -> outputToMotor(IntakeConfig.INTAKE_SPEED));
    }

    /** Spins intake wheels and belt to eject balls. */
    public Command spinBackward() {
        return Commands
                .runOnce(() -> outputToMotor(-IntakeConfig.INTAKE_SPEED));
    }
